...
def test_development_machine_help_lists_the_complete_sdk_machine_lifecycle():
...
def test_sdk_api_errors_are_mapped_and_credentials_are_redacted(monkeypatch):
    ...
    result = invoke(
        ["development-machine", "inspect", "--project", "demo", "machine-1"]
    )
...
def test_mocked_sdk_transport_for_every_supported_machine_operation(
    command, path, method, response, expected_request
):
