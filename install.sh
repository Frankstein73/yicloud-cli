...
readonly LEGACY_ZSH_BLOCK_BEGIN="# >>> yicloud-cli >>>"
readonly LEGACY_ZSH_BLOCK_END="# <<< yicloud-cli <<<"
...
remove_legacy_zsh_path_block() {
    ...
    [[ -f $profile ]] || return 0
    file_has_line "$profile" "$LEGACY_ZSH_BLOCK_BEGIN" || return 0
    ...
    while IFS= read -r line || [[ -n $line ]]; do
        if [[ $line == "$LEGACY_ZSH_BLOCK_BEGIN" ]]; then
            inside_block=1
            continue
        fi
        if [[ $line == "$LEGACY_ZSH_BLOCK_END" ]]; then
            inside_block=0
            continue
        fi
...
configure_shell_path() {
    ...
    if (( force_update == 0 )) && [[ :$PATH: == *:$bin_directory:* ]]; then
        return 0
    fi

    note "Adding $bin_directory to PATH for the detected shell..."
    if UV_TOOL_BIN_DIR="$bin_directory" uv tool update-shell; then
        return 0
    fi

    note "Warning: the shell PATH could not be updated automatically."
    note "Add $bin_directory to PATH using your shell's configuration, then open a new terminal."
    return 1
}
...
legacy_zsh_config=${YICLOUD_ZSH_CONFIG:-${ZDOTDIR:-$home_directory}/.zshrc}
...
if [[ ${1-} == "--uninstall" ]]; then
    ...
    require_command mktemp "mktemp is required to update legacy shell configuration safely."
    require_command cp "cp is required to update legacy shell configuration safely."
    ...
    # Releases before the shell-neutral installer wrote a yicloud-specific
    # block to .zshrc. Remove that legacy block when present, but leave uv's
    # shared PATH configuration intact because other uv tools may rely on it.
    if path_is_absolute "$legacy_zsh_config"; then
        remove_legacy_zsh_path_block "$legacy_zsh_config"
    fi
...
if path_is_absolute "$legacy_zsh_config" && file_has_line "$legacy_zsh_config" "$LEGACY_ZSH_BLOCK_BEGIN"; then
    if configure_shell_path "$bin_directory" 1; then
        remove_legacy_zsh_path_block "$legacy_zsh_config"
        note "Migrated the legacy yicloud-cli Zsh PATH block to shell-neutral PATH configuration."
    else
        note "The legacy yicloud-cli Zsh PATH block was left unchanged."
    fi
else
    configure_shell_path "$bin_directory" || true
fi
...
if [[ :$PATH: == *:$bin_directory:* ]]; then
    ...
    note "Open a new terminal, then run: yicloud --help"
    note "To verify it now without restarting your shell, run: $cli_link --help"
fi
